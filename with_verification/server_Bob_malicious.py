...
while True:
    ...
    def apply_multi_qubit_ops():
        ...
        for i, state_vector in enumerate(reconstructed_states):
            ...
            alpha, beta = state_vector
            theta = 2 * np.arccos(np.abs(alpha))       # use magnitude of alpha!
    ...
    dev2 = qml.device("default.qubit", wires=num_qubits, shots = 1)
    ...
    for x in range(1, n+1):
        for y in range(1, m+1):
            ...
            s = 0 if res == 1 else 1
            print(f"Qubit {c} measurement result: {s}\n")
